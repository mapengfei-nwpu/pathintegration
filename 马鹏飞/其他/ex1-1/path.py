...
def oderk4(moments,t,dt):
    ...
    def RHS(M10, M01, M11, M20, M02, TM, MARK):
        ALPHA = 1.0; BETA = 0.3; YETA = 0.1; FF = 0.2; OMIGA = 1.2; SEGMA2 = 0.01;
        # ALPHA = -1.0; BETA = 0.2; YETA = 0.2; FF = 0.0; OMIGA = 1.2; SEGMA2 = 0.5;
    ...
    for p in probability:
        if p<0:
            ...
            if p>0.16:
                ...
                z.append(0.05-0.15625*p)
    # bubble point
    points_new = bubblePoints(x, y, z, 3.0)
    #
        #
    #x_new=[];y_new=[];
    #for point in points_new:
...
# initial mesh grid
y   = np.linspace(-1,1,50)*3.0
x   = np.linspace(-1,1,50)*3.0
x,y = np.meshgrid(x,y)
x,y = x.ravel(),y.ravel()
